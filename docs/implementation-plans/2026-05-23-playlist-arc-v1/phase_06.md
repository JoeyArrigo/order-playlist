...
**Scope:** Phase 6 of 7 from `<project-root>/docs/design-plans/2026-05-23-playlist-arc-v1.md`.
...
**Project guidance:** `<project-root>/implementation-plan-guidance.md`. Same rules as Phase 5: no `unwrap()` outside tests/`main`; doc comments on `pub` items; AC9.1 (no silent skips); AC9.2 (`tracing::info!` on every network call).
...
Run: `cd <project-root> && cargo build --features reccobeats`
...
**Files:**
- Modify: `<project-root>/tests/support/in_memory.rs`
...
Run: `cd <project-root> && cargo test --test in_memory_features_smoke`
Expected: passes.
...
**Files:**
- Modify: `<project-root>/src/adapters/reccobeats.rs`
...
Run: `cd <project-root> && cargo test --features reccobeats --lib adapters::reccobeats::tests`
Expected: all pass.
...
Run: `cd <project-root> && cargo test --features reccobeats --lib adapters::reccobeats`
...
**Files:**
- Create: `<project-root>/tests/reccobeats_live.rs`
...
Without the feature: `cd <project-root> && cargo test --test reccobeats_live` → builds with zero tests.

With the feature: `cd <project-root> && cargo test --features reccobeats,live-network --test reccobeats_live` → passes if network reaches reccobeats.com. If it fails on this Task, **return to Task 1 spike** — the API has likely changed shape since the design was written.
